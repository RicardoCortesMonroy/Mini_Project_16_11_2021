...
def distance_constraint(point, centre, max_distance):
  return max_distance**2 - ((point[0]-centre[0])**2 + (point[1]-centre[1])**2) # If positive, ensures the point is within max_distance of centre
...
# Returns the given bounds centred around a point
def bounds_centred_on(centre, bounds = None, diameter = None):
    centred_bounds = np.empty((len(centre),2))
    if bounds:
        for i in range(len(centre)):
            bound_range = max(bounds[i]) - min(bounds[i])
            centred_bounds[i] = np.array([-bound_range/2, bound_range/2]) + centre[i]
        return centred_bounds
    elif diameter:
        for i in range(len(centre)):
            centred_bounds[i] = np.array([-diameter/2, diameter/2]) + centre[i]
        return centred_bounds
    else:
        raise ValueError("Must specify either bounds or radius")

# Returns centre of a given set of bounds
def centre_of_bounds(bounds):
...
def random_radial_sample(centre, radius):
    arg = 2*math.pi*random.random()
    mod = math.sqrt(radius**2*random.random())
    return np.array([mod*math.cos(arg), mod*math.sin(arg)]) + centre
...
def circle(centre, radius, resolution = 20):
    angle = np.linspace(0, 2*math.pi, resolution)
    x = radius * np.cos(angle) + centre[0]
    y = radius * np.sin(angle) + centre[1]
    return (x,y)
